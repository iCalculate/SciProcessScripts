...
%
% 数据格式需求:
%   RamanData.data           - 光谱矩阵 (行: 序列/功率, 列: 拉曼位移)
%   RamanData.axisscale{1,1} - 参数轴 (功率/角度/温度等)
%   RamanData.axisscale{2,1} - 拉曼位移轴 (cm⁻¹)
...
try
    ...
    % 自动查找 RamanData 结构或第一个结构体
    RamanData = [];
    if ismember('RamanData', fieldNames)
        RamanData = data.RamanData;
    else
        % 使用第一个结构体
        for i = 1:length(fieldNames)
            if isstruct(data.(fieldNames{i}))
                RamanData = data.(fieldNames{i});
                break;
            end
        end
    end
    ...
    % 验证数据结构
    if isempty(RamanData) || ~isstruct(RamanData)
        error('未找到有效的数据结构');
    end
    
    if ~isfield(RamanData, 'data') || ~isfield(RamanData, 'axisscale')
    ...
    numRamanPoints = size(spectra, 2);
    
    fprintf('✓ 数据加载成功\n');
    fprintf('  ├─ 曲线数量: %d\n', numCurves);
    fprintf('  ├─ 拉曼点数: %d\n', numRamanPoints);
    fprintf('  ├─ 参数范围: %.4f - %.4f\n', min(paramAxis), max(paramAxis));
    fprintf('  └─ 拉曼范围: %.0f - %.0f cm⁻¹\n\n', min(ramanAxis), max(ramanAxis));
...
switch plotMode
    case 'A'
        ...
        ax = axes('Parent', fig);
        plot(ax, ramanAxis, spectrum, 'b-', 'LineWidth', 2.5);
        set(ax, 'FontName', 'Arial', 'FontSize', 12);
        xlabel(ax, 'Raman Shift (cm^{-1})', 'FontSize', 13);
        ylabel(ax, 'Intensity (a.u.)', 'FontSize', 13);
        title(ax, sprintf('Raman Spectrum #%d (Parameter = %.6f)', idx, paramValue), 'FontSize', 13);
        grid(ax, 'on');
        box(ax, 'on');
    ...
    case 'B'
        ...
        subData.axisscale{2,1} = ramanAxis;
        ...
        normChoice = input('选择归一化方法 (1-4，默认 2): ');
        ...
        % 如果选择峰范围，让用户输入
        peakRange = [min(ramanAxis) + (max(ramanAxis) - min(ramanAxis)) * 0.4, ...
                     min(ramanAxis) + (max(ramanAxis) - min(ramanAxis)) * 0.6];
        
        if strcmp(normMethod, 'peak_range')
            fprintf('\n输入峰范围 (按 Enter 使用默认 [%.0f %.0f]):\n', peakRange(1), peakRange(2));
            peakMin = input('最小拉曼位移 (cm⁻¹): ');
            if ~isempty(peakMin)
                peakRange(1) = peakMin;
            end
            peakMax = input('最大拉曼位移 (cm⁻¹): ');
            if ~isempty(peakMax)
                peakRange(2) = peakMax;
            end
        end
        
        [fig, ax] = plot_raman_stack(subData, ...
            'Indices', 1:numSelected, ...
            'NormalizeMethod', normMethod, ...
            'PeakRange', peakRange, ...
            'Spacing', 0.1, ...
            'Colormap', 'orange', ...
            'Title', sprintf('Raman Spectra Stack (%d curves)', numSelected), ...
            'LineWidth', 1.5);
    ...
    case 'C'
        ...
        [fig, ax] = plot_raman_contour(subData, ...
            'ColorRange', [0.5 1.0], ...
            'ContourLevels', 50, ...
            'Colormap', 'hot', ...
            'Title', sprintf('Raman Intensity Map (%d curves)', numSelected));
    ...
    case 'D'
        ...
        for i = 1:numPlots
            ...
            plot(ax, ramanAxis, spectrum, 'b-', 'LineWidth', 1.5);
            set(ax, 'FontName', 'Arial', 'FontSize', 10);
            xlabel(ax, 'Raman Shift (cm^{-1})', 'FontSize', 11);
    ...
    case 'E'
        ...
        % 子图1: 堆叠图
        ax1 = subplot(1, 2, 1);
        [fig_stack, ax_stack] = plot_raman_stack(subData, ...
            'Indices', 1:numSelected, ...
            'NormalizeMethod', 'max', ...
            'Spacing', 0.1, ...
            'Colormap', 'orange', ...
        ...
        % 子图2: 等高线图
        ax2 = subplot(1, 2, 2);
        [fig_contour, ax_contour] = plot_raman_contour(subData, ...
            'ColorRange', [0.5 1.0], ...
            'ContourLevels', 50, ...
            'Colormap', 'hot', ...
            'Title', 'Intensity Map');
    ...
    case 'D'
        ...
        if saveName ~= 0
            ...
            try
                ...
                exportData.raman_shift = ramanAxis;
