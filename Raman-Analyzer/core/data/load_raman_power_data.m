function [RamanData] = load_raman_power_data(raman_filepath, power_filepath)
%LOAD_RAMAN_POWER_DATA 加载拉曼数据和激光功率数据
%   [RamanData] = load_raman_power_data(raman_filepath, power_filepath)
%   
%   输入:
%       raman_filepath - 拉曼数据文件路径
%       power_filepath - 激光功率数据文件路径
%   
%   输出:
%       RamanData - 合并后的数据结构
%           .data - 拉曼光谱数据矩阵
%           .axisscale{1,1} - 激光功率值 (mW)
%           .axisscale{2,1} - 拉曼位移 (cm⁻¹)

try
    RamanData = import_raman_data(raman_filepath);
catch ME
    error('拉曼数据加载失败: %s', ME.message);
end

numPowers = size(RamanData.data, 1);
...
if length(laserPower) ~= numPowers
    ...
    if minLen < numPowers
        warning('功率数据长度 (%d) 少于拉曼数据行数 (%d)。已截断。', ...
            length(laserPower), numPowers);
    else
        warning('功率数据长度 (%d) 多于拉曼数据行数 (%d)。已截断。', ...
            length(laserPower), numPowers);
    end
    RamanData.data = RamanData.data(1:minLen, :);
    laserPower = laserPower(1:minLen);
end

RamanData.axisscale{1,1} = laserPower;
