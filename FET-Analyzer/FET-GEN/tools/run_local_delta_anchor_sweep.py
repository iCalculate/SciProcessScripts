...
def run() -> Path:
    output_dir = ROOT / "experiments" / f"local-delta-anchor-{time.strftime('%Y%m%d-%H%M%S')}"
    ...
    for anchor_strength in [0.0, 0.25, 0.5, 0.75, 1.0]:
        checkpoint = output_dir / f"anchor_{int(round(anchor_strength * 100)):03d}.npz"
        ...
        results.append(
            {
                "name": f"anchor_{anchor_strength:.2f}",
