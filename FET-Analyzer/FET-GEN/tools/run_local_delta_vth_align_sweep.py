...
def run() -> Path:
    output_dir = ROOT / "experiments" / f"local-delta-vth-align-{time.strftime('%Y%m%d-%H%M%S')}"
    ...
    for strength in [0.10, 0.15, 0.20, 0.25, 0.30, 0.35]:
        checkpoint = output_dir / f"localdelta_vthalign_{int(round(strength * 100)):02d}.npz"
        ...
        payload.append(
            {
                "name": f"vth_align_{strength:.2f}",
                "description": "Direction-split local delta guide with reduced base latent scale and post Vth horizontal alignment.",
    ...
    ranked = payload[:1] + sorted(payload[1:], key=lambda item: _score({
        **item["jump_metrics"],
