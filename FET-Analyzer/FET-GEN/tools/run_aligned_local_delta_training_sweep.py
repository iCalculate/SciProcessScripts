...
def _specs() -> list[SweepSpec]:
    return [
        SweepSpec(
            name="aligned_delta_dirsplit24_balanced",
            description="Threshold-aligned delta PCA with balanced forward and reverse guides.",
            forward_config=_cfg(24, beta=0.018, subthreshold_weight=3.1, slope_weight=0.35, seed=35051),
            reverse_config=_cfg(24, beta=0.018, subthreshold_weight=3.1, slope_weight=0.35, seed=35052),
        ),
        SweepSpec(
            name="aligned_delta_dirsplit24_slopefocus",
            description="Threshold-aligned delta PCA with stronger local slope emphasis near Vth.",
            forward_config=_cfg(24, beta=0.014, subthreshold_weight=3.3, slope_weight=0.50, seed=35161),
            reverse_config=_cfg(24, beta=0.014, subthreshold_weight=3.3, slope_weight=0.50, seed=35162),
        ),
        SweepSpec(
            name="aligned_delta_dirsplit28_lowbeta",
            description="Higher-capacity threshold-aligned delta PCA with lighter latent regularization.",
            forward_config=_cfg(28, beta=0.010, subthreshold_weight=3.2, slope_weight=0.42, seed=35271),
            reverse_config=_cfg(28, beta=0.010, subthreshold_weight=3.2, slope_weight=0.42, seed=35272),
        ),
        SweepSpec(
            name="aligned_delta_dirsplit20_reversefocus",
            description="Keep the forward guide compact and put extra threshold smoothness into the reverse guide.",
            forward_config=_cfg(20, beta=0.016, subthreshold_weight=2.9, slope_weight=0.32, seed=35381),
            reverse_config=_cfg(24, beta=0.012, subthreshold_weight=3.6, slope_weight=0.52, seed=35382),
        ),
    ]
...
def run() -> Path:
    output_dir = ROOT / "experiments" / f"aligned-local-delta-training-sweep-{time.strftime('%Y%m%d-%H%M%S')}"
    ...
    lines = [
        "# Threshold-aligned delta PCA sweep",
        "",
